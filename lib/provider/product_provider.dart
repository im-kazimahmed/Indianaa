...
class ProductProvider extends ChangeNotifier {
 ...
 int fiveStarLength ,fourStar,threeStar,twoStar,oneStar;
  ...
  Future<void> getRelatedProductList(BuildContext context, int productId) async {
    ApiResponse apiResponse = await productRepo.getRelatedProductsList(productId);
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      if (offset == '1') {
        _relatedProductList = [];
      }
      var data = (apiResponse.response.data as List<dynamic>)
        ?.map((productJson) => Product.fromJson(productJson))
        ?.toList();
      _relatedProductList = data;
      _isLoading = false;
      notifyListeners();
    } else {
      showCustomSnackBar(apiResponse.error.toString(), context);
