...
class CartScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: ResponsiveHelper.isDesktop(context)? PreferredSize(child: WebAppBar(), preferredSize: Size.fromHeight(120)) :  CustomAppBar(title: getTranslated('my_cart', context), isBackButtonExist: fromDetails),
        ...
        builder: (context, cart, child) {
          ...
          return cart.cartList.length > 0 ? Column(
            children: [
                        ...
                        Center(
                          child: Container(
                            ...
                            child: Column(
                                ...
                                children: [
                                 if(!ResponsiveHelper.isDesktop(context)) productView(cart, _total, _couponController, context),
                              // Product
                                SizedBox(height: Dimensions.PADDING_SIZE_LARGE),

                                if(!ResponsiveHelper.isDesktop(context))  detailsView(_isSelfPickupActive, context, _kmWiseCharge, _itemPrice, _tax, _discount, deliveryCharge, _total, _couponController),
                              Row(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  if(ResponsiveHelper.isDesktop(context)) Expanded(
                                      child: Container(
                                        ...
                                        child: productView(cart, _total, _couponController, context),
                                      )),
                                  // Product
                                  ...
                                  if(ResponsiveHelper.isDesktop(context))  Expanded(
                                      child: Container(
                                        ...
                                        //margin: const EdgeInsets.symmetric(horizontal: Dimensions.PADDING_SIZE_SMALL,vertical: Dimensions.PADDING_SIZE_LARGE),
                                        padding: ResponsiveHelper.isDesktop(context) ?  EdgeInsets.symmetric(horizontal: Dimensions.PADDING_SIZE_LARGE,vertical: Dimensions.PADDING_SIZE_LARGE) : EdgeInsets.zero,
                                        child: detailsView(_isSelfPickupActive, context, _kmWiseCharge, _itemPrice, _tax, _discount, deliveryCharge, _total, _couponController),
                                      )),
                                ],
                              )
                              // Order type
             ...
             if(!ResponsiveHelper.isDesktop(context)) buttonView(context, _itemPrice, _total),
            ...
            child: Column(
              children: [
                ConstrainedBox(
                  constraints: BoxConstraints(minHeight: !ResponsiveHelper.isDesktop(context) && _height < 600 ? _height : _height - 400),
                  child: NoDataScreen(isCart: true),
                ),
                if(ResponsiveHelper.isDesktop(context)) FooterView(),
              ],
            ),
    ...
    return Container(
      ...
      child: CustomButton(btnTxt: getTranslated('continue_checkout', context), onTap: () {
        if(_itemPrice < Provider.of<SplashProvider>(context, listen: false).configModel.minimumOrderValue) {
          showCustomSnackBar('Minimum order amount is ${PriceConverter.convertPrice(context, Provider.of<SplashProvider>(context, listen: false).configModel.minimumOrderValue)}, you have ${PriceConverter.convertPrice(context, _itemPrice)} in your cart, please add more item.', context);
        } else {
  ...
  Column productView(CartProvider cart, double _total, TextEditingController _couponController, BuildContext context) {
    return Column(
      children: [
        ListView.builder(
        ...
        // Coupon
        if(!ResponsiveHelper.isDesktop(context)) cupponView(context, _couponController, _total)
      ],
    );
  }

  Column detailsView(bool _isSelfPickupActive, BuildContext context, bool _kmWiseCharge, double _itemPrice, double _tax, double _discount, double deliveryCharge, double _total, TextEditingController _couponController) {
    return Column(
      children: [
        if(ResponsiveHelper.isDesktop(context)) cupponView(context, _couponController, _total),
  ...
  Container cupponView(BuildContext context, TextEditingController _couponController, double _total) {
    return Container(
        decoration: BoxDecoration(
            color: Theme.of(context).cardColor,
            borderRadius: BorderRadius.circular(10),
